...
Сквозные сценарии без реальных мессенджеров: Telegram — на подменённой сессии aiogram, MAX — на локальном сервере с форматами из официальной схемы API. Проверяются запись, отмена, общее расписание, напоминания и ответы администратора.

```
python tests/e2e_telegram.py
python tests/e2e_max.py
```
...
tests/      — сквозные проверки обоих ботов
```
